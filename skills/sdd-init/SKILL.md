...
- If mode is `memory`:
  ...
  **Save project context**:
  ...
  curl -s -X POST http://127.0.0.1:8192/api/memories \
    ...
    -d '{"content": "{detected project context markdown}", "title": "sdd-init/{project-name}", "topic_key": "sdd-init/{project-name}", "type": "architecture", "project": "{project-name}", "tags": ["sdd-init", "{project-name}"]}'
  ```
  `topic_key` enables upserts — re-running init updates the existing context, not duplicates.

  (See `skills/_shared/memory-convention.md` for full naming conventions.)
- If mode is `obsidian`:
  Read and follow `skills/_shared/obsidian-convention.md`. Auto-detect Obsidian CLI (`obsidian --version` exits 0). If unavailable, fall back to direct filesystem writes.

  **Create vault structure**:
  ```bash
  # With CLI
  obsidian create name="sdd/init/{project-name}" folder="sdd/init" content="{frontmatter + project context}"

  # Without CLI (filesystem fallback)
  mkdir -p "{vault-root}/sdd/init"
  # Write sdd/init/{project-name}.md with frontmatter (see obsidian-convention.md)
  ```

  Do NOT create `openspec/` directory.
- If mode is `openspec`: Read and follow `skills/_shared/openspec-convention.md`. Run full bootstrap.
- If mode is `hybrid`: Read and follow BOTH convention files. Run openspec bootstrap AND persist context to Memory.
...
4. If memory service is available, **ALSO save to memory**:
  ```bash
  curl -s -X POST http://127.0.0.1:8192/api/memories \
    -H "Content-Type: application/json" \
    -d '{"content": "{registry markdown}", "title": "skill-registry", "topic_key": "skill-registry", "type": "config", "project": "{project}", "tags": ["config", "skill-registry"]}'
  ```
...
curl -s -X POST http://127.0.0.1:8192/api/memories \
  -H "Content-Type: application/json" \
  -d '{"content": "{your detected project context from Steps 1-4}", "title": "sdd-init/{project-name}", "topic_key": "sdd-init/{project-name}", "type": "architecture", "project": "{project-name}", "tags": ["sdd-init", "{project-name}"]}'
```

If mode is `openspec` or `hybrid`: the config was already written in Step 3.

If mode is `hybrid`: also POST to memory as above (write to BOTH backends).
...
Persist project context following `skills/_shared/memory-convention.md` with title and topic_key `sdd-init/{project-name}`.
...
### Context Saved
Project context persisted to Memory (mcp-memory-service).
- **Memory ID**: {memory-id}
...
### Recommendation
Start the memory server (`memory launch`) or enable `openspec` for artifact persistence across sessions. Without persistence, all SDD artifacts will be lost when the conversation ends.
