...
2. Search Memory API: `POST /api/memories/search` with `{"query": "skill-registry", "tags": ["config"], "limit": 3}` → `GET /api/memories/{id}` for full content
...
- The registry lives in memory/filesystem, not in the orchestrator's context
