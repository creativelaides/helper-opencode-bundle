...
name: skill-registry
description: >
  Create or update the skill registry for the current project. Scans user skills and project conventions, writes .atl/skill-registry.md, and saves to memory if available.
...
#### B. If memory service is available, also save to memory (cross-session bonus):

```bash
curl -s -X POST http://127.0.0.1:8192/api/memories \
  -H "Content-Type: application/json" \
  -d '{"content": "{registry markdown from Step 3}", "title": "skill-registry", "topic_key": "skill-registry", "type": "config", "project": "{project}", "tags": ["config", "skill-registry"]}'
```
...
- ALWAYS save to memory if the Memory service is available (http://127.0.0.1:8192 returns healthy)
