...
- **memory**: Read `sdd/{change-name}/proposal`, `sdd/{change-name}/spec`, `sdd/{change-name}/design`, `sdd/{change-name}/tasks` (all required — keep tasks ID for updates). Mark tasks complete via upsert (POST with same `topic_key` containing updated checklist). Save progress as `sdd/{change-name}/apply-progress`.
- **obsidian**: Read and follow `skills/_shared/obsidian-convention.md`. Read all upstream artifacts via CLI or filesystem. Write `sdd/{change-name}/apply-progress.md` with frontmatter. Update `sdd/{change-name}/tasks.md` marking `- [x]` for completed tasks (Obsidian Tasks plugin compatible). **Daily notes integration**: after each significant action, append to today's daily note:
  ```bash
  obsidian daily:append content="- [x] sdd/{change-name}: Completed task {id} - {description}"
  ```
  If Obsidian CLI unavailable, use direct filesystem writes for all operations.
- **openspec**: Read and follow `skills/_shared/openspec-convention.md`. Update `tasks.md` with `[x]` marks.
- **hybrid**: Follow BOTH conventions — persist progress to Memory AND update `tasks.md` with `[x]` marks on filesystem.
...
- type: `architecture`
- Also update the tasks artifact with `[x]` marks via Memory API upsert (memory) or file edit (openspec/hybrid).
